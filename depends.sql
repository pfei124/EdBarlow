...
|*
|* Author: Q Vincent Yin (umyin@mctrf.mb.ca)
|*
|* Description: It's a superset of sp_depends.
|*
|* Usage:  sp__depends
|*
...
begin
    ...
    */
    ...
    if @objname is NULL
         begin
              select 'object'=convert(varchar(30),object_name(id)),
                     'dependant'=convert(varchar(30),object_name(depid))
              from  sysdepends
              return
         end
    ...
    if @id != 0 /* found */
    begin
    select @msg='/* '+@objname+' is a usertype. */'
    print @msg
        if not exists (select * from syscolumns where usertype = @id)
        begin
            print '/* Datatype isnt used by any object. */'
        end
        ...
        begin
            ...
            where c.usertype = @id and
                  c.id = o.id
            order by object, c.name
        end

        if @format = 'drop'
        begin
            select @msg='exec sp_droptype '''+@objname+''''+@go
            print @msg
        end
        return
    end
    ...
    begin
        ...
        begin
            if @type = 'U'
             begin
                select @msg='drop table '''+@objname+''''+@go
                print @msg
             end
            if @type = 'V'
             begin
                select @msg='drop view '''+@objname+''''+@go
                print @msg
             end
            else if @type = 'P'
             begin
                select @msg='drop proc '''+@objname+''''+@go
                print @msg
             end
            else if @type = 'TR'
             begin
                select @msg='drop trigger '''+@objname+''''+@go
                print @msg
             end
        end
    end
    ...
    begin
        select @msg= '/* '+@objname+' is a default. */'
        print @msg
        ...
        if @format = 'drop'
        begin
            select @msg='drop default '+@objname
            print @msg
            print 'go'
        end
    end
    ...
    begin
        select @msg='Cant handle object type '+ @type
        print @msg
        return 1
    end
