...
if exists (select * from sysobjects
            where name = "sp__helpindex"
              and type = "P")
   drop procedure sp__helpindex

go

create procedure sp__helpindex (
                                @object char(30)=NULL,
                                @width int=80,
                                @srvname char(30)=NULL,
                                @show_type char(1)=' ',
                                @dont_format char(1) = null,
                                @no_print char(1) = null
			       )
...
begin
   ...
   select owner        = user_name(o.uid),
          ...
          status       = i.status, status2=i.status2
...
if @width>0 and @srvname is null and @no_print is null
begin
   print "   INDEX KEY:     c = clustered            u = unique"
   print "                  a = allow dup row        s = suspect"
   print "                  i = ignore dup key "
