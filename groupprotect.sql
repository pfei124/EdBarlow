...
/************************************************************************\
|* Procedure Name: sp__groupprotect
|*
|* Author:
|*
|* Description: permissions by object type / user group vs sel/upd/ins/del
|*
|* Usage:       sp__groupprotect
|*
...
if exists (select * from sysobjects
           ...
           and    type = "P")
begin
   drop procedure sp__groupprotect
end
go
...
if @dont_format is null
  begin
    select type,grp=convert(char(15),group_name),tot=convert(char(6),total),sel=convert(char(6),s),
           upd=convert(char(6),u),del=convert(char(6),d),ins=convert(char(6),i),
	   rev=convert(char(6),r),exe=convert(char(6),e)
      from #objects
     order by type,group_name
  end
else
  begin
    select type,grp=group_name,tot=convert(char(6),total),sel=convert(char(6),s),
           upd=convert(char(6),u),del=convert(char(6),d),ins=convert(char(6),i),
	   rev=convert(char(6),r),exe=convert(char(6),e)
      from #objects
     order by type,group_name
