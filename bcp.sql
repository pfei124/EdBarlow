...
|* Author:
|*
|* Description: Produces bcp script to copy out tables from database
|*    For each table
|*    BCP [db]..[table] out [table].dat -U[login] -P[password] -S[server] -c [commands]
|*
|*    If no parameters are supplied, the values for login, server,
|*    file ext, and database are found from the available information
...
if exists (select *
           ...
           and    name = "sp__bcp")
begin
    drop procedure sp__bcp
end
go

create procedure sp__bcp (
                          @server        char(30) = NULL,
...
begin
    ...
    if @server is null
        ...
        print '# Usage is...'
                ...
                print '#                        @@servername may not be defined)'
    ...
    begin
      ...
      from   #tab
           ...
           if  @direction='out'
                        ...
                        and ( select row_count(db_id(), i.id)
                                        from sysindexes i
