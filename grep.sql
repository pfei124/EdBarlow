...
|*              Correctly handles cases when a substring begins in
...
create procedure sp__grep (@parameter varchar(255) = null, 
	                   @case char(1) = 's',
...
begin
  ...
  begin
      ...
      begin
          select @context = rtrim( substring(
                      @parameter + space( 255 - datalength( @parameter) ), 1, 20 ) )
          select @msg='Search string '+ @msg_str_no +' is missing, before "'+ @context + '".'
          print @msg
          return
      end
      if @oper_pos > 81
...
select  Name        = convert(char(50), o.name),
