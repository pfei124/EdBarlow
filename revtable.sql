...
|*
|* Date        Who     What
|* 04.11.2010  kra20   some modifications
|*  
\************************************************************************/
...
if exists (select *
           from   sysobjects
           where  type = "P"
           and    name = "sp__revtable")
begin
    drop procedure sp__revtable
end
go

create procedure sp__revtable (@objname char(50)=NULL)
AS
BEGIN
 SET NOCOUNT ON
 ...
 IF @objname IS NULL
 BEGIN
  SELECT convert(varchar(70), "Usage: sp__revtableOK @objname")
    return -1
 END
 ...
 BEGIN
        ...
        @buffer=' ' + substring(c.name+space(30),1,@maxnamelen) + t.name +
                -- ***************** is too short for fields > 999 ************ kra20
                --  + substring('(' + convert(varchar(3),c.length) + ')',     --kra20
                    + substring('(' + convert(varchar(5),c.length) + ')', 1, 6 * (charindex(t.name, 'varchar/varbinary'))),
        @scale   = c.scale,
        @prec    = c.prec,
        @fldname = t.name,
        @status  = c.status,
        @type    = c.type ,
             -- ***************** produces corrupt create statement *** kra20
             -- @dflt  = ISNULL(OBJECT_NAME(c.cdefault),'None'),      --kra20
             -- @roole = ISNULL(OBJECT_NAME(c.domain),'None')         --kra20
                @dflt  = 'None',                                      --kra20
                @roole = 'None'                                       --kra20
          ...
          BEGIN
            SELECT @tdflt = m.text
             ...
             WHERE c.id = object_id(@objname)
               ...
               AND o.id = m.id
            SELECT @maxdfltlen = char_length(rtrim(@tdflt))+2
          END
        ELSE
            SELECT @tdflt = null,@maxdfltlen = 0
        IF @roole != 'None'
          BEGIN
            SELECT @troole = 'ALTER TABLE ' + rtrim(@objname) + ' ADD '+ rtrim(m.text)
	...
	IF (@fldname != "bit" AND @status & 8 != 0)
	   SELECT @buffer = substring(@buffer+space(24-@maxdfltlen),1,@maxnamelen+30) + 'NULL'
        ELSE
  	   SELECT @buffer = substring(@buffer+space(24-@maxdfltlen),1,@maxnamelen+30) + 'NOT NULL'
	IF @cnt < @maxcnt
  	   SELECT @buffer = rtrim(@buffer) + ','
